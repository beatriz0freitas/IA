...
class InterfaceMapa(tk.Canvas):
    ...
    def desenhar_nos(self):
        ...
        for no in self.grafo.nos.values():
            ...
            self.create_text(x, y - NODE_RADIUS - 10, text=no.id_no, font=("Arial", 9, "bold"), tags="nos")
    ...
    def draw_legend(self, x=10, y=10):
        ...
        spacing = 26
        for i, (_, label, color, shape) in enumerate(linhas):
            yy = y + i * spacing
            if shape == "oval":
                self.create_oval(x, 
                                 yy, 
                ...
                self.create_oval(x, 
                                 ...
                                 outline="black", 
                                 tags="legenda")
                self.create_text(x + 6, 
                                 yy + 6, 
                                 font=("Arial", 7), 
                                 tags="legenda")
            self.create_text(x + 22, 
                             yy + 7, 
                             text=label, 
                             anchor="w", 
                             font=("Arial", 9), 
    ...
    def atualizar_veiculos(self, veiculos: dict):
        ...
        for v in veiculos.values():
            ...
            rect = self.create_rectangle(x - VEHICLE_HALF, 
                                         y - VEHICLE_HALF, 
                                         x + VEHICLE_HALF, 
                                         y + VEHICLE_HALF,
                                         fill=color, 
            ...
            self.create_text(x, y + 14, text=v.id_veiculo, font=("Arial", 8), tags="veiculos")

            # Se o veículo tem rota, desenha uma linha da posição atual até próximo destino (visual feedback)
            if getattr(v, "rota", None):
                coords = [self._pos(node) for node in v.rota]
                for a, b in zip(coords[:-1], coords[1:]):
                    self.create_line(a[0], a[1], b[0], b[1], fill=color, dash=(4, 3), width=2, tags="rotas")


    def atualizar(self, veiculos: dict, pedidos: list):
        self.desenhar_grafo()  # redesenha nós/arestas caso a viewport mude (barato para grafo pequeno)
