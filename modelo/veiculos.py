"""
Modelo de veículo (elétrico ou a combustão) - CORRIGIDO
...
class Veiculo(ABC):
    ...
    id_rota: int
    rota: list = field(default_factory=list)  # rota atual (lista de nós)
    indice_rota: int = field(default=0)  # ✓ ADICIONADO: índice na rota
    ...
    def move(self, distancia_km: float, no_destino: str):
        """Atualiza posição após percorrer uma distância"""
    ...
    def definir_rota(self, rota: list):
        """Define nova rota e reseta índice"""
        self.rota = rota
        self.indice_rota = 0  # ✓ IMPORTANTE: reseta índice

    def mover_um_passo(self, grafo: Grafo):
        """
        Move o veículo para o próximo nó na rota.
        Retorna False se a rota terminou, True se o movimento foi bem-sucedido.
        """
        ...
        if self.indice_rota >= len(self.rota) - 1:
            self.estado = EstadoVeiculo.DISPONIVEL
            return False
