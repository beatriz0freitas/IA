...
class GestorFrota:
    ...
    def definir_algoritmo_procura(self, nome: str):
        """Define algoritmo de procura: astar, ucs, bfs ou dfs."""
        if nome.lower() not in ("astar", "ucs", "bfs", "dfs"):
            raise ValueError(f"Algoritmo inválido: {nome}")
        self.algoritmo_procura = nome.lower()
        # Limpa cache de rotas ao mudar algoritmo
        self.cache_rotas.limpar_cache()
